	...
	<script language="JavaScript">
		function validate() {
			...
			if(f.tl_loginname.value=="") {
				alert('You need to enter your ' + f.databasename.value + ' database login name (For Normal Operation)!');
				return false;
			}

			if(f.tl_loginpassword.value=="") {
				alert('You need to enter your ' + f.databasename.value + ' database password (For Security empty password is not allowed)!');
				return false;
			}
...
<?php if(!$_SESSION['isNew']){ ?>
	...
	<p>
		Warning! You have requested an Upgrade, this process will MODIFY your current TestLink Database. <br/>
...
<?php } ?>
		...
		<p>Define your database to store <?php echo $_SESSION['app_name']?> data:</p>
		...
		<p>
   		<div class="tlBox">Note: In the case that your DB connection dosn't use <b>STANDARD PORT</b> for ,
    		you need to add '<b>:port_number</b>', at the end of Database host parameter.
    		For example: you use MySQL running on port 6606, on server matrix 
    		then Database host will be <i>matrix:6606</i>
		</div>
		</p>			
		<p>Enter the name of the <?php echo $_SESSION['app_name']?> database <?php if(!$_SESSION['isNew'])echo " for upgrade."; else
    		...
    		<input type="text" id="databasename" name="databasename"  maxlength="50" 
    					       style="width:200px" value="<?php echo strtolower($_SESSION['app_name'])?>">
		</p>
		<?php if($_SESSION['isNew']){ ?>
		<p>
		...
		<div class="tlBox">
			...
			<?php echo $_SESSION['app_name']?> can be installed (using this installer) on a existing database 
			used by another application, using a table prefix.<br />
			Warning! PART OF INSTALLATION PROCESS CONSISTS 
			on dropping all <?php echo $_SESSION['app_name']?> tables present on the database/schema (if any <?php echo $_SESSION['app_name']?> table exists). 
		...
		<p>
			...
			<input type="text" id="databaseloginname" name="databaseloginname" style="width:200px" value="root"/><br />
			<div class="labelHolder"><label for="databaseloginpassword">Database admin password</label></div>
			<input type="password" id="databaseloginpassword" name="databaseloginpassword" style="width:200px" value="root"/><br />
		</p>
		<p>
		<div class="tlBox">
		...
		<p>Define database User for <?php echo $_SESSION['app_name']?> DB access:</p>
			<div class="labelHolder">
				<label for="tl_loginname"><?php echo $_SESSION['app_name']?> DB login</label>
			</div>
			<input type="text" id="tl_loginname" name="tl_loginname" style="width:200px" value="chendelin"/><br />
					
			<div class="labelHolder">
				<label for="tl_loginpassword"><?php echo $_SESSION['app_name']?> DB password</label>
			</div>
			<input type="password" id="tl_loginpassword" name="tl_loginpassword" style="width:200px" value="chendelin"/><br />
		</p>
		<p>
		<div class="tlBox">
			This user will have permission only to work on <?php echo $_SESSION['app_name']?> database and will be 
			stored in <?php echo $_SESSION['app_name']?> configuration.<br />
    	    All <?php echo $_SESSION['app_name']?> requests to the Database will be done with this user.
		</div>
		</p>
				
		<p>
			<?php if($_SESSION['isNew'])echo 'After successfull installation You will' .
						' have the following login account for ' .$_SESSION['app_name'] .' Administrator:<br />' .
				        '<b>login name: admin</b> <br /><b>password  : admin</b>';
			?>
		</p>
		<p>
			<input type="submit" value="Process <?php echo $_SESSION['app_name']?> Setup!">
		</p>
