<?php
	...
	$smarty_detail->template_dir = 'sysapp/appmanage/templates/';
	...
	//验证是否有权限
	else if(!checkPermissions($db_detail, 1)){
	...
	switch($ac){
		case 'ajaxEdit':
			...
			if($id == ''){
				$set[] = "dt = now()";
				$smarty_detail->insert(0, 0, 'tb_app', $set);
			}else{
				$sqlwhere = "and tbid = $id";
				$smarty_detail->update(0, 0, 'tb_app', $set, $sqlwhere);
			}
			break;
		case 'uploadimg':
			//别人的http上传文件类
			require_once('sysapp/common/inc/upload.class.php');
			//文件目录
		...
		default:
			if(isset($appid)){
				$rs = $db_detail->select(0, 1, 'tb_app', '*', 'and tbid='.$appid);
				$smarty_detail->assign('app', $rs);
			}
			$smarty_detail->assign('apptype', $apptype);
